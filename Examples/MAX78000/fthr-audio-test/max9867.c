...
#define MAX9867_MXIN_LINE_INPUT 2
...
int max9867_init(mxc_i2c_regs_t *i2c_inst, int mclk, int lrclk)
{
    ...
    */
    ...
    // Select input
    if ((err = reg_write(MAX9867_14_ADC_INPUT, (MAX9867_MXIN_LINE_INPUT << MAX9867_MXINL_POS) | (MAX9867_MXIN_LINE_INPUT << MAX9867_MXINR_POS))) != E_NO_ERROR) return err;
    ...
    // Set playback volume
    if ((err = reg_write(MAX9867_10_VOL_LEFT, 0x09 << MAX9867_VOL_POS)) != E_NO_ERROR) return err;
    if ((err = reg_write(MAX9867_11_VOL_RIGHT, 0x09 << MAX9867_VOL_POS)) != E_NO_ERROR) return err;
